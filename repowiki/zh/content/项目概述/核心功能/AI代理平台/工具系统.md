...
participant LoopCheck as "循环检测"
Hook->>Runtime : 获取会话状态
Hook->>LoopCheck : detectToolCallLoop(state, toolName, params)
alt 循环严重
LoopCheck-->>Hook : level=critical, message
Hook-->>Hook : 阻止调用并记录
else 正常
Hook->>LoopCheck : recordToolCall(state, toolName, params)
Hook-->>Hook : 继续执行
end
```
